        ...
        if(data){
            db.updateInfohash(data).then(function(msg){
                if(msg==="success"){
                    ...
                    db.insertMetadata(infoHash,data.info).catch(()=>{
                        resaveArr[infoHash]?resaveArr[infoHash]++:resaveArr[infoHash]=0;
                        if(resaveArr[infoHash]<4){
                            setTimeout(()=>{
                                    _this.saveMetadata(infoHash,metadata);

                            },3000)
                        }else{
                            delete resaveArr[infoHash];
                        }
                    })
                }else{
                    _this.saveAvailable=true;
