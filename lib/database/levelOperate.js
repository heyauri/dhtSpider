...
let db_metadata = level(metadata_path, {maxFileSize: 67108864, cacheSize: 67108864, writeBufferSize: 34108864});
let db_infohash = level(infohash_path, {maxFileSize: 67108864, cacheSize: 128108864, writeBufferSize: 128108864});
    ...
    indexToLog() {
        ...
        let stream = db_infohash.createReadStream()
            .pipe(through(function (obj) {
                ...
                if(obj.key.indexOf('001_')>-1){
                    let arr=JSON.parse(obj.value);
                    let str=arr.join(',');
                    obj.key=obj.key+'_'+arr.length;
                    obj.value=str;
