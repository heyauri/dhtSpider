...
let indexSearch= function(str){
    ...
    return new Promise((resolve,reject)=>{
        Promise.all(indexPromiseArr).then(async (values)=>{
            ...
            indexesArr=indexesArr.concat(keywords);
            indexesArr=deduplicate(indexesArr);
            let names=await getNames(indexesArr);
            let result=[];
            let threshold=0;
            if(names.length>100){
                threshold=names.length*0.002;
            }
...
let indexClean=function(indexesArr,target,threshold,option){
    let result=[];
    if(threshold&&option){
        threshold+=0.02;
    }
        ...
        threshold=indexesArr.length*0.0015+0.35;
        threshold>0.75?threshold=0.75:threshold;
    }
    threshold>0.96?threshold=0.96:threshold;
    ...
    if(result.length>50&&threshold<0.96){
        return indexClean(indexesArr,target,threshold,true);
    }
    else if(result.length<3&&option===true){
        return indexClean(indexesArr,target,threshold,false);
    }
    else return result;
};
...
let getIndex=function(keyword){
    ...
    let langs=langjudge.langAllContain(keyword);
    let items=[];
    if(langs.indexOf('English')||langs.indexOf('Latin')||langs.indexOf('Cyrillic')){
        items.push(shortKeys[Math.floor(shortKeys.length/2)]);
    }else{
        items=shortKeys;
    }
    let threshold=3;
